...
function LearningReport({ monthly }: { monthly: MonthlyLearningMetric[] }) {
  ...
  return <section className="learning-report" aria-label="月ごとの学習レポート"><div className="learning-report__heading"><div><span className="eyebrow eyebrow--gold">YOUR LEARNING RHYTHM</span><h2>学習レポート</h2><p>{encouragement}</p></div><div className="learning-report__totals"><span>直近6か月</span><strong>{minutesLabel(totalMinutes)}</strong><small>完了 {totalCompleted}講座</small></div></div><div className="learning-report__stats"><article><span>今月の学習時間</span><strong>{minutesLabel(current?.watchedMinutes ?? 0)}</strong></article><article><span>今月の視聴完了</span><strong>{current?.completedCount ?? 0}講座</strong></article><article><span>学習継続</span><strong>{hasActivity ? "記録中" : "これから"}</strong></article></div><div className="learning-chart" role="img" aria-label="直近6か月の学習時間の棒グラフ">{monthly.map(item => <div key={item.key} className="learning-chart__column"><div className="learning-chart__value">{item.watchedMinutes > 0 ? minutesLabel(item.watchedMinutes) : "—"}</div><div className="learning-chart__track"><span style={{ height: `${Math.max(item.watchedMinutes > 0 ? 8 : 0, (item.watchedMinutes / maxMinutes) * 100)}%` }} /></div><strong>{item.label}</strong><small>{item.completedCount > 0 ? `完了 ${item.completedCount}` : ""}</small></div>)}</div><p className="learning-report__note">学習時間は、視聴位置を保存した実際の再生時間に基づいて集計されます。</p></section>;
}

function RecommendationPanel({ recommendations }: { recommendations: Recommendation[] }) {
  if (!recommendations.length) return null;
  const labels = { continue: "続きから学ぶ", topic: "理解を深める", explore: "新しいテーマ" } as const;
  return <section className="recommendation-panel" aria-label="あなたへのおすすめ講座"><div className="recommendation-panel__heading"><div><span className="eyebrow eyebrow--gold">PERSONALIZED NEXT STEPS</span><h2>次に視聴するとよい講座</h2><p>視聴履歴・完了講座・テーマの傾向から、まだ完了していない講座を提案しています。</p></div><span className="recommendation-panel__count">{recommendations.length}件の提案</span></div><div className="recommendation-grid">{recommendations.map(({ course, reason, kind }) => <Link key={course.id} href={`/courses/${course.slug}`} className="recommendation-card"><CourseArtwork theme={course.thumbnailTheme} category={course.category.name} title={course.title} compact /><div className="recommendation-card__body"><span className="recommendation-card__tag">{labels[kind]}</span><h3>{course.title}</h3><p className="recommendation-card__reason">{reason}</p><div className="recommendation-card__meta"><span>{course.category.name}</span><span>{course.durationMinutes}分</span><strong>視聴する</strong></div></div></Link>)}</div><p className="recommendation-panel__note">おすすめはご自身の保存済み視聴データだけを使用し、医療上の個別判断や診療の提案は行いません。</p></section>;
}

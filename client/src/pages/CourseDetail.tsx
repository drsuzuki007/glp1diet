...
export default function CourseDetail() {
  ...
  const subscribe = trpc.subscription.activateDemo.useMutation({
    onSuccess: result => { utils.catalog.actions.invalidate(); utils.subscription.mine.invalidate(); utils.library.mine.invalidate(); setSubscriptionOpen(false); toast.success(result.alreadySubscribed ? "すでに加入済みです" : "サブスクリプションに加入しました。すべての講座を視聴できます。"); },
    onError: () => toast.error("加入処理を完了できませんでした。もう一度お試しください。"),
  });
  ...
  return <SiteFrame>
    ...
    {subscriptionOpen && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="サブスクリプション加入確認"><div className="purchase-modal"><button className="modal-close" onClick={() => setSubscriptionOpen(false)} aria-label="閉じる"><X size={20} /></button><span className="eyebrow eyebrow--gold">DEMO SUBSCRIPTION</span><h2>全講座見放題に加入する</h2><p className="purchase-modal__course">glp1.diet の全講座を、いつでも視聴できます。</p><div className="purchase-modal__total"><span>月額料金（税込）</span><strong>{formatYen(monthlyPrice)}</strong></div><div className="demo-note"><CheckCircle2 size={18} /><p>これは実課金のないデモ加入です。カード会社への送信、請求、実際の金銭の移動は一切発生しません。</p></div><button className="button-primary button-primary--full" onClick={() => subscribe.mutate()} disabled={subscribe.isPending}>{subscribe.isPending ? "処理中…" : "デモ加入を完了する"}</button><button className="text-button" onClick={() => setSubscriptionOpen(false)}>キャンセル</button></div></div>}
    {playerOpen && <div className="modal-backdrop" role="dialog" aria-modal="true" aria-label="学習プレーヤー"><div className="player-modal"><button className="modal-close" onClick={() => setPlayerOpen(false)} aria-label="閉じる"><X size={20} /></button><div className="player-modal__screen"><video ref={playerVideoRef} className="learning-video" src={previewVideoUrl} controls autoPlay loop playsInline onLoadedMetadata={event => { const savedSeconds = actions?.progress?.lastPositionSeconds ?? 0; if (savedSeconds > 0 && savedSeconds < event.currentTarget.duration) event.currentTarget.currentTime = savedSeconds; }} onTimeUpdate={event => setDemoProgress(calculateVideoProgress(event.currentTarget.currentTime, event.currentTarget.duration))} aria-label={`${course.title}の学習プレーヤー`} /></div><div className="player-modal__body"><span className="eyebrow eyebrow--gold">LEARNING PLAYER</span><h2>{course.title}</h2><div className="progress-line"><span style={{ width: `${demoProgress || actions?.progress?.progressPercent || 0}%` }} /></div><div className="player-modal__meta"><span>進捗 {demoProgress || actions?.progress?.progressPercent || 0}%</span><span>{course.durationMinutes}分</span></div><button className="button-primary button-primary--full" onClick={saveProgress} disabled={progress.isPending}><HeartPulse size={17} />{progress.isPending ? "保存中…" : "現在の視聴位置を保存する"}</button><p>このデモ環境では、加入後の視聴導線と進捗保存を検証するための映像を再生しています。</p></div></div></div>}
  </SiteFrame>;
}

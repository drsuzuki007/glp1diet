...
For the personalized recommendation check, the active learner's My Page displayed three `続きから学ぶ` suggestions based on 51%, 18%, and 10% unfinished progress. The newly completed `lab-values-guide` course was excluded. Each suggestion included the saved percentage, its themed category, duration, and a direct view link; the UI also states that the recommendation uses only the learner's own stored history and does not provide medical advice.

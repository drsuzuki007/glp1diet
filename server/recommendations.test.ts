...
const courses = [
  ...
  { id: 2, slug: "b", title: "GLP-1応用", durationMinutes: 35, publishedAt: new Date("2026-08-10T00:00:00Z"), category: { id: 1, slug: "glp1-basics", name: "GLP-1の基礎" } },
...
describe("buildRecommendations", () => {
  ...
  it("offers new themes when the learner has no viewing history", () => {
    ...
    expect(recommendations.map(item => item.course.id)).toEqual([1, 3]);
  ...
  it("uses only the remaining goal after another goal is removed", () => {
    const recommendations = buildRecommendations(courses, [{ id: 1, progressPercent: 20, completed: false }], 3, ["improve_lifestyle"]);
    expect(recommendations.map(item => item.course.id)).toEqual([1, 3, 2]);
    expect(recommendations.slice(1).every(item => !item.reason.includes("GLP-1の基礎を知る"))).toBe(true);
    expect(recommendations[1]?.reason).toContain("食事・生活習慣を整える");
  });
});
